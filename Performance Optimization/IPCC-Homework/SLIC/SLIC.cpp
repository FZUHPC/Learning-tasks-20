	...
	const int&					NUMITR)
{
	...
	while( numitr < NUMITR )
	{
		...
		y1 = max(0,			(int)(kseedsy[n]-offset));
		if(ymax <= y1)	ymax = y1;
		y2 = min(m_height,	(int)(kseedsy[n]+offset));
		if(ymin >= y2) ymin = y2;
		} 
		
		int x1 = max(0,			(int)(kseedsx[0]-offset));
		int x2 = min(m_width,	(int)(kseedsx[0]+offset));
		...
		for( int y = ymin; y < ymax; y++ )
		{
			...
			x1 = max(0,			(int)(kseedsx[n]-offset));
			x2 = min(m_width,	(int)(kseedsx[n]+offset));
			}
